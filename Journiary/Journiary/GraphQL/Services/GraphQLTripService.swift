...
class GraphQLTripService: ObservableObject {
    ...
    private var isDemoMode: Bool {
        ...
        print("🔍 Demo Mode Check: Backend URL = \(backendURL), isDemo = \(isDemo)")
        return isDemo
    }
    ...
    func createTrip(
        ...
        print("🚀 Trip erstellen gestartet: \(name)")
        
        if isDemoMode {
            print("📱 Demo Mode: Trip wird in Core Data erstellt")
        ...
        print("🌐 Backend Mode: Trip wird via GraphQL erstellt")
        ...
        print("📤 Sende GraphQL CreateTrip Mutation mit Input: \(inputDict)")
        ...
        .tryMap { response in
            print("📥 CreateTrip Response erhalten")
            ...
            guard let trip = TripDTO.from(graphQL: tripDict) else {
                print("❌ Fehler beim Parsen der Trip-Daten: \(tripDict)")
                throw GraphQLError.serverError("Fehler beim Parsen der erstellten Trip-Daten")
            }
            print("✅ Trip erfolgreich erstellt: \(trip.name)")
            return trip
        }
        .mapError { error in
            print("❌ Fehler beim Trip erstellen: \(error)")
            if let graphQLError = error as? GraphQLError {
                return graphQLError
            } else {
                return GraphQLError.networkError(error.localizedDescription)
