...
class SimpleSyncMonitoringSystem extends events_1.EventEmitter {
    ...
    async getPerformanceAnalysis(timeWindow = 'hour') {
        ...
        const relevantMetrics = this.metrics.filter(m => m.timestamp && new Date(m.timestamp) >= windowStart);
        const analysis = {
    ...
    async performHealthCheck() {
        ...
        try {
            ...
            if (overallScore < 0.7) {
                await this.triggerAlert('health', overallScore < 0.5 ? 'critical' : 'warning', `System health degraded: ${(overallScore * 100).toFixed(1)}%`, healthCheck.details);
            }
            return {
    ...
    async triggerAlert(type, severity, message, details) {
        ...
        // Duplikat-Detection
        const isDuplicate = this.alerts.some(existing => existing.type === type &&
    ...
    async analyzeTrends(days = 7) {
        ...
        const relevantMetrics = this.metrics.filter(m => m.timestamp && new Date(m.timestamp) >= startDate);
        const trends = {
    ...
    async analyzeMetricRealtime(metric) {
        ...
        if (metric.duration > 5000) { // > 5 Sekunden
            await this.triggerAlert('performance', 'warning', `Slow operation detected: ${metric.operation} took ${metric.duration}ms`, metric);
        }
        // Fehlgeschlagene Operationen
        if (!metric.success) {
            await this.triggerAlert('error', 'warning', `Operation failed: ${metric.operation} - ${metric.errorMessage || 'Unknown error'}`, metric);
        }
        // Sehr hohe Entity-Counts
        if (metric.entityCount > 1000) {
            await this.triggerAlert('throughput', 'info', `High throughput operation: ${metric.operation} processed ${metric.entityCount} entities`, metric);
        }
    }
    cleanupOldData() {
        const cutoff = new Date(Date.now() - 24 * 60 * 60 * 1000); // 24 Stunden
        // Alte Metriken entfernen
        this.metrics = this.metrics.filter(m => m.timestamp && new Date(m.timestamp) >= cutoff);
        // Alte Alerts entfernen (die bereits acknowledged sind)
        this.alerts = this.alerts.filter(a => !a.acknowledged || (Date.now() - a.timestamp.getTime()) < 7 * 24 * 60 * 60 * 1000 // 7 Tage
        );
    ...
    detectAnomalies() {
        ...
        if (currentDuration > avgDuration * 3) {
            this.triggerAlert('performance', 'warning', `Performance anomaly detected: Current duration ${currentDuration}ms vs average ${avgDuration.toFixed(0)}ms`);
        }
    }
    ...
    calculateErrorRateTrend(metrics) {
        if (metrics.length < 10)
            return 'insufficient-data';
        const half = Math.floor(metrics.length / 2);
        const firstHalf = metrics.slice(0, half);
        ...
        if (secondErrorRate < firstErrorRate * 0.8)
            return 'improving';
        if (secondErrorRate > firstErrorRate * 1.2)
            return 'degrading';
        return 'stable';
    }
    calculateThroughputTrend(metrics) {
        if (metrics.length < 10)
            return 'insufficient-data';
        const half = Math.floor(metrics.length / 2);
        const firstHalf = metrics.slice(0, half);
    ...
    generateTrendRecommendations(metrics) {
...
exports.SimpleSyncMonitoringSystem = SimpleSyncMonitoringSystem;
